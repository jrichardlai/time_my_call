...
public class RubotoView extends android.view.View {
  ...
  public boolean onFilterTouchEventForSecurity(android.view.MotionEvent event) {
    if (callbackProcs[CB_FILTER_TOUCH_EVENT_FOR_SECURITY] != null) {
      try {
        return (Boolean)RuntimeHelpers.invoke(getRuby().getCurrentContext(), callbackProcs[CB_FILTER_TOUCH_EVENT_FOR_SECURITY], "call" , JavaUtil.convertJavaToRuby(getRuby(), event)).toJava(boolean.class);
      } catch (RaiseException re) {
        re.printStackTrace();
        return false;
      }
  ...
  public void onOverScrolled(int scrollX, int scrollY, boolean clampedX, boolean clampedY) {
    if (callbackProcs[CB_OVER_SCROLLED] != null) {
      try {
        IRubyObject[] args = {JavaUtil.convertJavaToRuby(getRuby(), scrollX), JavaUtil.convertJavaToRuby(getRuby(), scrollY), JavaUtil.convertJavaToRuby(getRuby(), clampedX), JavaUtil.convertJavaToRuby(getRuby(), clampedY)};
        RuntimeHelpers.invoke(getRuby().getCurrentContext(), callbackProcs[CB_OVER_SCROLLED], "call" , args);
      } catch (RaiseException re) {
        re.printStackTrace();
      }
    }
